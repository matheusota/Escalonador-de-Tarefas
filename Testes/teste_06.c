// Este teste é o Dummy User, disponibilizado em outras vezes anteriores em que a matéria MC404 foi dada
/*
 user.c
 -
...
void codigo_processo_2(int param_for) {
	...
	for (i = 0; i < 10; ++i) {
		...
		pbuf = buf;
		pbuf = pbuf + str2str(pbuf, "PID ");
		pbuf = pbuf + int2string(pbuf, (int) getpid());
		pbuf = pbuf + str2str(pbuf, ": Estou vivo!\n");
		write(1, buf, (int) (pbuf - buf));    
	}
	adeus();
}
